...
- [ ] **SCRAPE-01**: ~~woff 字体加密解码（Boss直聘反爬）~~ → **已移至 v2**（反爬复杂度过高，v1 不做 Boss直聘/实习僧）
- [ ] **SCRAPE-02**: v1 搜索互联网大厂**官方招聘页**（字节/阿里/腾讯/美团/百度等内置列表），通过 Tavily API 搜索 + 内容抓取；Boss直聘/实习僧 → **v2**
- [ ] **SCRAPE-03**: 搜索前询问用户偏好（城市、岗位类型、行业、关键词补充），结合简历自动推断用户意向，制定搜索策略
- [ ] **SCRAPE-04**: Tavily 搜索某公司失败时，展示失败原因，提供手动粘贴 JD 的降级入口（按公司维度）
